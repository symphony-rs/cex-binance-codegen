...
pub mod encoder {
    ...
    impl<'a> KlinesResponseEncoder<'a> {
        ...
        #[inline]
        pub fn klines_encoder(self, count: u32, klines_encoder: KlinesEncoder<Self>) -> KlinesEncoder<Self> {
    ...
    impl<'a, P> Writer<'a> for KlinesEncoder<P> where P: Writer<'a> + Default {
        #[inline]
        fn get_buf_mut(&mut self) -> &mut WriteBuf<'a> {
    ...
    impl<'a, P> Encoder<'a> for KlinesEncoder<P> where P: Encoder<'a> + Default {
        #[inline]
        fn get_limit(&self) -> usize {
    ...
    impl<'a, P> KlinesEncoder<P> where P: Encoder<'a> + Default {
        #[inline]
        pub fn wrap(
            mut self,
            mut parent: P,
...
pub mod decoder {
    ...
    impl<'a, P> ActingVersion for KlinesDecoder<P> where P: Reader<'a> + ActingVersion + Default {
        #[inline]
        fn acting_version(&self) -> u16 {
    ...
    impl<'a, P> Reader<'a> for KlinesDecoder<P> where P: Reader<'a> + Default {
        #[inline]
        fn get_buf(&self) -> &ReadBuf<'a> {
    ...
    impl<'a, P> Decoder<'a> for KlinesDecoder<P> where P: Decoder<'a> + ActingVersion + Default {
        #[inline]
        fn get_limit(&self) -> usize {
            self.parent.as_ref().expect("parent missing").get_limit()
        }
    ...
    impl<'a, P> KlinesDecoder<P> where P: Decoder<'a> + ActingVersion + Default {
        pub fn wrap(
            mut self,
            mut parent: P,
        ) -> Self {
